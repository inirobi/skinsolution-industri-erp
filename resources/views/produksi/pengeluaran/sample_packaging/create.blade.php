...
<div class="page-title">
  <div class="title_left">
    <h3>Add New Pengeluaran Sample Packaging</h3>
  </div>
    ...
    <div style='float:right'>
      <div class="input-group">
        <ul class="breadcrumb">
          ...
          <li><a href="{{route('pengeluaran_sample_pck.index')}}">Pengeluaran Sample Packaging</a></li>
          @if(isset($packout))
            <li>Update Pengeluaran Sample Packaging</li>
          @else
            <li>Add Pengeluaran Sample Packaging</li>
          @endif
        </ul>
      </div>
    </div>
...
<div class="row">
  <div class="col-md-12 col-sm-12">
    <div class="x_panel">
      <div class="x_content">
        @if(isset($packout))
          <form action="{{ route('pengeluaran_sample_pck.update', $packout->id) }}" novalidate method="POST" enctype="multipart/form-data">
          @method('PUT')
        @else
            <form action="{{ route('pengeluaran_sample_pck.store') }}" method="POST" enctype="multipart/form-data">          
        @endif
        @csrf
          ...
          <span class="section">
            @if(isset($packout))
              {{ __('Update Pengeluaran Sample Packaging') }}
            @else
              {{ __('Add Pengeluaran Sample Packaging') }}
            @endif
          </span>
          ...
          <div class="field item form-group">
              ...
              <div class="col-md-6 col-sm-6">
                  <select class="form-control @error('sample_packaging_id') is-invalid @enderror" name="sample_packaging_id">
                  @if(isset($packout))
                    @foreach($packaging as $bv)
                      <option @if($bv->id == $packout->sample_packaging_id) selected @endif value="{{$bv->id}}" >{{$bv->category}} - {{$bv->packaging_name}}</option>
                    @endforeach
                  @else
                    @foreach($packaging as $bv)
