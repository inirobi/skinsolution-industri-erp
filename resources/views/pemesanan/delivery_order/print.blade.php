...
<html>
	...
	<body>
		...
		<article>
			<h1>Bill To : </h1>
			<address contenteditable>
				<p>{{$inv->customer->customer_name}}</p>
				<p>{{$inv->customer->customer_mobile}}</p>
			</address>
			<h1>Ship To : </h1>
			<address contenteditable>
				<span class="address-head mb-5">{{$inv->customer->customer_name}}</span>
				<span>{{$inv->customer->customer_address}}</span><br>
				<abbr title="Phone">P:</abbr>{{$inv->customer->customer_mobile}}
			</address>

			<table class="meta">
				<tr>
					<th><span contenteditable>Number #</span></th>
					<td><span contenteditable>{{$inv->delivery_order_num}}</span></td>
				</tr>
				<tr>
					<th><span contenteditable>Date</span></th>
					<td><span contenteditable>{{$inv->date}}</span></td>
				</tr>
			</table>
			<?php 
				$leftOver = 0;
...
<!--			
			...
			<table class="inventory">
				...
				<tbody>
					@if(!empty($poProduct))
					@foreach($poProduct as $p)
						@foreach($delivery_order_view as $dataDetail)
						@if(!empty($poProduct) && $p->product_id == $dataDetail->product_id)
						<?php 
							$gtotal = $p->quantity;
							foreach ($delivery_order_view as $dataDetail) {
								if($p->product_id == $dataDetail->product_id && !empty($dataDetail->product_id)){
									$gtotal = $p->quantity - $dataDetail->quantity;
								}				
							}
							foreach ($po as $dataDetail) {
								if($p->product_id == $dataDetail->product_id && !empty($dataDetail->product_id)){
									$gtotal = $gtotal - $dataDetail->quantity;
								}				
							}
							$num++;
						?>
